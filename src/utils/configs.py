...
class TrainingConfig():
    def __init__(self, gpu,noveld_coefficent,episode_len):
    ...
    def init_logger(self):
        if self.group_name is not None:
            self.wandb_run = wandb.init(
                name=f'run-id-{self.run_id}-{self.start_datetime}',
        ...
        self.save_model_log_dir = os.path.join('./save_models', self.env_name, str(self.run_id), self.start_datetime)
        os.makedirs(self.save_model_log_dir, exist_ok=True)
        self.model_log_dir = os.path.join(self.log_dir, self.env_name, str(self.run_id), self.start_datetime)
        ...
        print(f'Starting run {self.run_id}---{self.env_name}')
