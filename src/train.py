...
def train(config):
    ...
    model = PPOTrainer(
        ...
        noveld_coefficent = config.noveld_coefficent,
        first_state=config.first_state,
        episode_len=config.episode_len,
        save_model_log_dir = config.save_model_log_dir
    )
...
@click.option('--total_steps', default=int(6e7), type=int, help='Total number of frames to run for training')
...
@click.option('--game_name', default="ObstructedMaze-1Q", type=str, help='MultiRoom-N7-S8e.g. DoorKey-8x8, ninja, jumper')
...
@click.option('--can_see_walls', default=0, type=int, help='Whether walls are visible to the agent')
@click.option('--fully_obs', default=0, type=int, help='Whether the agent can receive full observations')
@click.option('--image_noise_scale', default=0.1, type=float, help='Standard deviation of the Gaussian noise')
...
##---------------------------------------******-----------------------------##
@click.option('--int_rew_source', default='TNCOA', type=str,
              help='Source of IRs: [TNCOA|NoModel|DEIR|ICM|RND|NGU|NovelD|PlainDiscriminator|PlainInverse|PlainForward|RIDE]')
...
@click.option('--dsc_obs_queue_len', default=100000, type=int, help='Maximum length of observation queue')
...
@click.option('--env_render', default=1, type=int, help='Whether to render games in human mode')
...
@click.option('--gpu', default=0, type=int,
              help='gpu number')
@click.option('--noveld_coefficent', default=1.0, type=float,
              help='noveld_coefficent ')
@click.option('--first_state', default=1, type=int,
              help='using first_state to limit')
@click.option('--episode_len', default=100, type=int,
              help='episode_len')
def main(
    ...
    rec_interval, video_length, log_dsc_verbose, env_render, use_status_predictor, gpu,noveld_coefficent,first_state,episode_len
):
    set_random_seed(run_id, using_cuda=True)
    args = locals().items()
    config = TrainingConfig(gpu, noveld_coefficent,episode_len)
