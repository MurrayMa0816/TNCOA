...
class PPORolloutBuffer(BaseBuffer):
    ...
    def reset(self) -> None:
        ...
        self.inrew_obs_dists = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)        
        self.inrew_novelty_dists = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)       
    ...
    def add(
        ...
        action: np.ndarray,
        reward: np.ndarray,
        inrew_obs_dists: np.ndarray,
        inrew_novelty_dists: np.ndarray,
        ...
        self.inrew_obs_dists[self.pos] = np.array(inrew_obs_dists).copy()  
        self.inrew_novelty_dists[self.pos] = np.array(inrew_novelty_dists).copy()             
