...
class IntrinsicRewardBaseModel(nn.Module):
    ...
    def _init_optimizers(self) -> None:
        ...
        self.model_params = [
            param for name, param in param_dicts
            if (name.find('status_predictor') < 0 and \
                name.find('position_predictor') < 0 and \
                name.find('model_mlp_novelD.noveld_rnd_model.target') < 0)
        ]
