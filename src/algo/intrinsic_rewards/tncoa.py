...
import src.algo.intrinsic_rewards.models as models
from src.utils.running_mean_std import RunningMeanStd, RunningMeanStdTorch
...
class DiscriminatorModel(IntrinsicRewardBaseModel):
    def __init__(
        ...
        # self.random_target_network = MinigridMLPTargetEmbeddingNet()#.to(device=flags.device) 
        # self.predictor_network = MinigridMLPEmbeddingNet()#.to(device=flags.device) 
        self.rnd_loss_coef = rnd_loss_coef
        self.predictor_learning_rate = predictor_learning_rate
        # self.predictor_optimizer = torch.optim.Adam(self.predictor_network.parameters(), 
                                                    # lr=self.predictor_learning_rate)
        self.scale_fac = scale_fac
        self.rnd_err_norm = rnd_err_norm
        self.rnd_err_momentum = rnd_err_momentum
        # self.rnd_err_running_stats = RunningMeanStd(momentum=self.rnd_err_momentum)RunningMeanStdTorch
        self.rnd_err_running_stats = RunningMeanStdTorch(momentum=self.rnd_err_momentum)
        ...
        self.noveld_beta = torch.tensor(0.0)
    def _build(self):
        # Build CNN and RNN
        super()._build()
        ...
        self.model_mlp_novelD = NovelDOutputHeads(
    ...
    def _init_modules(self) -> None:
        super()._init_modules()
        # module_name.update({self.model_mlp_novelD:'model_NovelD'})
        init_module_with_name('model_NovelD', self.model_mlp_novelD)
    ...
    # from Noveld
    def forward(self, curr_obs: Tensor, next_obs: Tensor, last_mems: Optional[Tensor], curr_dones: Tensor,curr_act: Tensor,):
        # curr_embs, curr_mems = self._get_rnd_embeddings(curr_obs, last_mems,self.model_rnns)
        # next_embs, _ = self._get_rnd_embeddings(next_obs, curr_mems, self.model_rnns)
        curr_embs, curr_mems = self._get_rnd_embeddings(curr_obs, last_mems)
        next_embs, _ = self._get_rnd_embeddings(next_obs, curr_mems)

        curr_tgt, curr_prd, next_tgt, next_prd = self.model_mlp_novelD(curr_embs, next_embs)
    ...
    def _get_training_losses(self,
        ...
        likelihoods = self.model_mlp(curr_embs, next_embs, curr_act)
        likelihoods = likelihoods.view(-1)        
        ...
        noveld_rnd_loss, noveld_curr_rnd_losses, noveld_next_rnd_losses, model_mems = \
                self.forward(curr_obs[n_half_batch:], next_obs[n_half_batch:], last_mems[n_half_batch:], curr_dones[n_half_batch:], curr_act[n_half_batch:])
        total_loss = dsc_loss + noveld_rnd_loss
        ...
        return dsc_loss, noveld_rnd_loss, total_loss, pos_dsc_loss, neg_dsc_loss, \
    ...
    def get_intrinsic_rewards(self,
        curr_obs: Tensor, next_obs: Tensor, last_mems: Tensor,
        obs_history: List, novelty_history: List, trj_history: List, curr_dones: Tensor, curr_act: Tensor, noveld_coefficent:float, first_state:int, plain_dsc: bool = False
    ):
        ...
        with th.no_grad():
            ...
            noveld_rnd_loss, noveld_curr_rnd_losses, noveld_next_rnd_losses, model_mems = \
                self.forward(curr_obs, next_obs, last_mems, curr_dones,curr_act)
        ...
        inrew_obs_dists = np.zeros(batch_size, dtype=np.float32)  
        inrew_novelty_dists = np.zeros(batch_size, dtype=np.float32)  
        for env_id in range(batch_size):
    
            novelty_list = novelty_history[env_id]
            curr_novelty = noveld_curr_rnd_losses[env_id].view(1, -1)
            next_novelty = noveld_next_rnd_losses[env_id].view(1, -1)
            new_novelty = [curr_novelty, next_novelty] if novelty_list is None else [novelty_list, next_novelty]
            novelty_list = th.cat(new_novelty, dim=0)
            novelty_history[env_id] = novelty_list

            novelty_dist = novelty_list[-1] - self.scale_fac * novelty_list[:-1]

            obs_dists_hist = torch.clamp(novelty_dist,min=0)
            if first_state:
                ...
                if not curr_dones[env_id]:
                    curr_obs_hash = curr_obs[env_id].reshape(-1).tolist()
                    curr_obs_hash_action = curr_obs_hash + [float(curr_act[env_id].tolist())]
                    obs_hash = tuple(curr_obs_hash_action)
            ...
            if not plain_dsc:
                ...
                deir_dists = obs_dists_hist * obs_dists / (trj_dists + 1e-6)
                int_rews[env_id] += deir_dists.min().item() 
                inrew_obs_dists[env_id] += obs_dists.mean().item() 
                inrew_novelty_dists[env_id] += obs_dists_hist.mean().item()
            else:
                # Plain discriminator (DEIR without the MI term)
                int_rews[env_id] += obs_dists.min().item()

        return inrew_obs_dists, inrew_novelty_dists, int_rews, model_mems
    ...
    def optimize(self, rollout_data, stats_logger):
        ...
        dsc_loss, noveld_rnd_loss, total_loss, pos_dsc_loss, neg_dsc_loss, \
        key_loss, door_loss, pos_loss, \
        key_dist, door_dist, goal_dist, \
        n_valid_samples, n_valid_pos_samples, n_valid_neg_samples, \
        avg_likelihood, pos_avg_likelihood, neg_avg_likelihood, dsc_accuracy = \
