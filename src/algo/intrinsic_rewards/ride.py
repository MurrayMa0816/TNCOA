...
class RIDEModel(IntrinsicRewardBaseModel):
    def __init__(
        self,
    ...
    def forward(self,
        ...
        #Intrinsic reward
        int_rews = torch.norm(next_embs - curr_embs, dim=1, p=2)

        return int_rews, fwd_losses, fwd_loss, inv_loss, curr_mems, 


    def get_intrinsic_rewards(self, curr_obs, next_obs, last_mems, curr_act, curr_dones, stats_logger):
        with th.no_grad():
            int_rews, icm_fwd_losses, fwd_loss, inv_loss, _ = \
        ...
        int_rews = int_rews.clone().cpu().numpy()
