...
class PPORollout(BaseAlgorithm):
    ...
    def __init__(
        ...
        noveld_coefficent: float = 0.5,
        first_state: int = 1,
        episode_len: int =100,
        save_model_log_dir:str =None
    ):
        ...
        self.noveld_coefficent = noveld_coefficent,
        self.first_state=first_state
        self.ep_info_buffer = None
        self.episode_len=episode_len
        self.save_model_log_dir=save_model_log_dir
    ...
    def on_training_start(self):
        ...
        self.episodic_novelty_history = [None for _ in range(self.n_envs)]
        self.episodic_trj_emb_history = [None for _ in range(self.n_envs)]

        if self.int_rew_source in [ModelType.TNCOA, ModelType.PlainDiscriminator]:
    ...
    def clear_on_episode_end(self, dones, policy_mems, model_mems):
        for env_id in range(self.n_envs):
            if dones[env_id]:
                ...
                self.episodic_novelty_history[env_id] = None
    ...
    def log_on_rollout_end(self, log_interval):
        if log_interval is not None and self.iteration % log_interval == 0:
            ...
            if self.int_rew_coef > 0:
                log_data.update({
                    "rollout/inrew_obs_dists": np.mean(self.ppo_rollout_buffer.inrew_obs_dists),
                    "rollout/inrew_novelty_dists": np.mean(self.ppo_rollout_buffer.inrew_novelty_dists),
            ...
            #     self.local_logger.writerow([self.iteration] + list(self.global_visit_counts.flatten()), log_type='global_visit_counts')
        # Log visualized data
    ...
    def create_intrinsic_rewards(self, new_obs, actions, dones):
        ...
        if self.int_rew_source in [ModelType.TNCOA, ModelType.PlainDiscriminator]:
            inrew_obs_dists,inrew_novelty_dists, intrinsic_rewards, model_mems = self.policy.int_rew_model.get_intrinsic_rewards(
                curr_obs=curr_obs_tensor,
                next_obs=next_obs_tensor,
                last_mems=last_model_mem_tensor,  
                obs_history=self.episodic_obs_emb_history,
                novelty_history=self.episodic_novelty_history,
                trj_history=self.episodic_trj_emb_history,
                curr_dones=done_tensor,      
                curr_act=curr_act_tensor, 
                noveld_coefficent=self.noveld_coefficent,     
      ...
      # DEIR / Plain discriminator model
        ...
        # RIDE    
        elif self.int_rew_source == ModelType.RIDE:
            intrinsic_rewards, model_mems = self.policy.int_rew_model.get_intrinsic_rewards(
                curr_obs=curr_obs_tensor,
                next_obs=next_obs_tensor,
                last_mems=last_model_mem_tensor,
        ...
        return intrinsic_rewards, model_mems, inrew_obs_dists,inrew_novelty_dists
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            # IR Generation
            intrinsic_rewards, model_mems, inrew_obs_dists,inrew_novelty_dists = \
    ...
    def learn(
        ...
        self.ep_info_buffer = deque(maxlen=self.episode_len)
        # #####
        # if self.ep_inrew_buffer is None :
        #     # Initialize buffers if they don't exist, or reinitialize if resetting counters
        #     self.ep_inrew_buffer = deque(maxlen=100)
        #     self.ep_obs_dist_buffer = deque(maxlen=100)
        ...
        while self.num_timesteps < total_timesteps:
            ...
            print(f'{time_now}  '
                  ...
                  f'rew: {rew_mean:.6f}  '
                  f'inrew_obs_dists: {np.mean(self.ppo_rollout_buffer.inrew_obs_dists).tolist():.6f}  '    
                  f'inrew_novelty_dists: {np.mean(self.ppo_rollout_buffer.inrew_novelty_dists).tolist():.6f}  '       
                  f'inrew: {(np.mean(self.ppo_rollout_buffer.intrinsic_rewards).tolist()):.6f}  '                  
                  f'rollout: {collect_end_time - collect_start_time:.3f} sec  '
                  f'train: {train_end_time - train_start_time:.3f} sec')
            
            model_path = os.path.join(self.save_model_log_dir, 'model.pth')
